...
%% %%%%%%%%% Case 1 %%%%%%%%%%%%%%%%%%%%%%%%
start=1;
NC_max=4; % Maximum number of capacitors
%% parameters setting
a=numel(LSI1(:,1))/2;
a=round(a);
...
[value1,index1]=max([particle.cost]);
[value,index]=min([particle.cost]);
...
for t=1:T
    %% Modified code
    c1=(c1i-c1f)*((T-t)/T)+c1f; %%c1
    c2=(c2i-c2f)*((T-t)/T)+c2f; %%c2
    ...
    b=0.02;
    r1=1;r2=1;
    E=(abs(Fmax)-abs(Fmin))/abs(Fmax); %%E
    w=1-((a)/(1+exp(b*E))); %%W
    
    for i=1:NP
         
          particle(i).velocity=w*particle(i).velocity + (1-w)*particle(i).velocity...
                              +c1*r1*(bparticle(i).pos-particle(i).pos)...
                              +c2*r2*(gparticle.pos-particle(i).pos);
